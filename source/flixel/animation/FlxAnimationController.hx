...
class FlxAnimationController implements IFlxDestroyable
{
  ...
  /**
   * The global speed of the animation controller.
   * All framerates are multiplied by this if followGlobalSpeed is enabled.
   */
  public static var globalSpeed:Float = 1;

  /**
   * Should all animation framerates be multiplied by globalSpeed?
   * Defaults to true.
   */
  ...
  public function update(elapsed:Float):Void
  {
    if (_curAnim != null)
    {
      ...
      {
        _curAnim.update((elapsed * (timeScale * FlxG.animationTimeScale)) * globalSpeed);
      }
      else
      {
  ...
  public function add(name:String, frames:Array<Int>, frameRate = 30.0, looped = true, flipX = false, flipY = false):Void
  {
    ...
    while (i-- >= 0)
    {
      ...
      if (frame >= numFrames)
      {
        ...
        if (framesToAdd == frames) framesToAdd = frames.copy();
    ...
    if (framesToAdd.length > 0)
    {
      ...
      if (hasInvalidFrames) FlxG.log.warn('Could not add frames above ${numFrames - 1} to animation: "$name"');
    }
    else
  ...
  public function append(name:String, frames:Array<Int>):Void
  {
    ...
    for (frame in frames)
    {
      if (frame < numFrames) anim.frames.push(frame);
      else
        hasInvalidFrames = true;
    }

    if (hasInvalidFrames) FlxG.log.warn('Could not append frames above ${numFrames - 1} to animation: "$name"');
  }
  ...
  function findByPrefix(animFrames:Array<FlxFrame>, prefix:String, logError = true):Void
  {
    ...
    #if FLX_DEBUG
    if (invalidFrames.length == 0 || !logError) return;
  ...
  function removeInvalidFrames(frames:Array<FlxFrame>)
  {
    ...
    {
      final frame = frames[i];
      if (frame.parent.shader == null) invalid.unshift(frames.splice(i, 1)[0]);
    }
