...
/**
 * FlxBackdrop, but extending PixelPerfectSprite instead of FlxSprite.
 */
class PixelPerfectBackdrop extends PixelPerfectSprite
{
  /**
  ...
  {
    if (repeatAxes == XY) return true;

    if (repeatAxes == NONE) return super.isOnScreen(camera);
  ...
  function drawToLargestCamera()
  {
    ...
    if (largest != null) regenGraphic(largest);
  }

  override function isSimpleRenderBlit(?camera:FlxCamera):Bool
  {
    if (repeatAxes == NONE) return super.isSimpleRenderBlit(camera);
  ...
  override function drawSimple(camera:FlxCamera):Void
  {
    ...
    if (drawDirect) tileSize.addPoint(spacing);
    ...
    if (FlxG.renderBlit) calcFrame(true);
