...
import ui.Alphabet.Letter;
import ui.Alphabet.Alignment;
...
class OptionsFont extends FlxSpriteGroup
{
  public var text(default, set):String;

  public var bold:Bool = false;
  public var letters:Array<OptionsCharacter> = [];
  ...
  public function clearLetters()
  {
    ...
    {
      --i;
      var letter:OptionsCharacter = letters[i];
      if (letter != null)
      {
        letter.kill();
  ...
  public function createLetters(newText:String)
  {
    ...
    for (character in newText.split(''))
    {
      if (character != '\n')
      {
        ...
        var isAlphabet:Bool = OptionsCharacter.isTypeAlphabet(character.toLowerCase());

        if (OptionsCharacter.allLetters.exists(character.toLowerCase()) && (!bold || !spaceChar))
        {
          ...
          var letter:OptionsCharacter = new OptionsCharacter(xPos, rows * Y_PER_ROW * scaleY, character, bold, this);
...
class OptionsCharacter extends FlxSprite
{
  ...
  public function new(x:Float, y:Float, character:String, bold:Bool, parent:OptionsFont)
  {
    super(x, y);
    this.parent = parent;
